...
class TestStripeWebhook:
    ...
    async def test_handles_checkout_session_completed(self):
        ...
        mock_checkout.assert_awaited_once_with(event.data, mock_session, "evt_checkout")
    ...
    async def test_handles_subscription_updated(self):
        ...
        assert result == {"status": "success"}
        mock_updated.assert_awaited_once_with(event.data, mock_session, "evt_updated")
    ...
    async def test_handles_subscription_deleted(self):
        ...
        assert result == {"status": "success"}
        mock_deleted.assert_awaited_once_with(event.data, mock_session, "evt_deleted")
...
class TestHandleCheckoutCompleted:
    ...
    async def test_upgrades_user_when_subscription_found(self):
        ...
        with (
            ...
            mock_analytics.return_value = MagicMock()
            await _handle_checkout_completed(data, session, stripe_event_id="evt_test")
...
class TestHandleSubscriptionUpdated:
    ...
    async def test_updates_user_tier_via_stripe_client(self):
        ...
        with (
            ...
            await _handle_subscription_updated(data, session, stripe_event_id="evt_upd")
    ...
    async def test_falls_back_to_raw_status_when_no_client(self):
        ...
        with (
            ...
            await _handle_subscription_updated(data, session, stripe_event_id="evt_fallback")
    ...
    async def test_supports_legacy_customer_field(self):
        ...
        with (
            ...
            await _handle_subscription_updated(data, session, stripe_event_id="evt_legacy")
    ...
    async def test_supports_legacy_id_field(self):
        ...
        with (
            ...
            await _handle_subscription_updated(data, session, stripe_event_id="evt_legacyid")
...
class TestHandleSubscriptionDeleted:
    ...
    async def test_downgrades_user_to_free_tier(self):
        ...
        with (
            ...
            await _handle_subscription_deleted(
                {"customer_id": "cus_del"}, session, stripe_event_id="evt_del"
            )
    ...
    async def test_captures_analytics_on_deletion(self):
        ...
        with (
            wp_patch,
            patch("app.api.subscriptions.get_analytics", return_value=mock_analytics_instance),
            patch("app.api.subscriptions.track_stripe_conversion", new_callable=AsyncMock),
        ):
            await _handle_subscription_deleted(
                {"customer_id": "cus_analytics"}, session, stripe_event_id="evt_analytics"
            )
