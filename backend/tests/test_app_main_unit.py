...
class TestLifespan:
    ...
    # Patches for jwt helpers that are imported locally inside lifespan()
    # They must be patched at their source module paths, not app.main.
    _JWT_PATCHES = (
        "app.security.get_jwt_auth_instance",
        "forge_shared.auth.dependencies.set_jwt_auth",
    )

    def _jwt_mock_stack(self):
        """Return a list of (patch_target, mock) tuples for JWT helpers."""
        return [
            patch("app.security.get_jwt_auth_instance", return_value=MagicMock()),
