...
# Check if database is available for CLI tests that require it
DB_AVAILABLE = bool(os.getenv("TEST_DATABASE_URL") or os.getenv("DATABASE_URL"))

# Resolve the interview-sim binary: prefer the current venv, fall back to PATH.
_VENV_BIN = Path(sys.executable).parent
_INTERVIEW_SIM_BIN = str(_VENV_BIN / "interview-sim")
...
def run_cli(cmd: list[str], expect_success: bool = True) -> dict:
    ...
    """
    # Replace "interview-sim" with the resolved venv binary so the CLI runs
    # even when the venv is not activated in the calling environment.
    resolved_cmd = [
        _INTERVIEW_SIM_BIN if arg == "interview-sim" else arg for arg in cmd
    ]

    # Add --json if not present
    if "--json" not in resolved_cmd:
        resolved_cmd.append("--json")

    result = subprocess.run(resolved_cmd, capture_output=True, text=True, check=False)
...
class TestGenerateQuestions:
    ...
    @pytest.mark.skipif(not DB_AVAILABLE, reason="Database required for generate-questions")
...
class TestExportMetrics:
    ...
    @pytest.mark.skipif(not DB_AVAILABLE, reason="Database required for export-metrics")
