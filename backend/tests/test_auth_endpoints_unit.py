...
class TestLogout:
    ...
    async def test_logout_unauthenticated_raises_401(self):
        ...
        Updated for S152 Week 2: get_current_user no longer accepts an `auth`
        parameter — it resolves ForgeAuth internally via _get_forge_auth().
        Passing credentials=None triggers the 401 path directly.
        """
        from app.dependencies import get_current_user

        with pytest.raises(HTTPException) as exc_info:
            await get_current_user(credentials=None, session=_mock_db_session())
