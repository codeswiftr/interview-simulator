...
6. Security headers - via rate limit middleware (forge-shared SecurityMiddleware handles the rest)
...
class TestRefreshTokenHashing:
    ...
    def test_create_refresh_token_returns_jwt_string(self):
        ...
        # Mock the ForgeAuth instance to avoid needing real settings
        mock_auth = MagicMock()
        mock_auth.create_refresh_token.return_value = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.test"

        with patch("app.security.get_forge_auth_instance", return_value=mock_auth):
...
class TestSecurityHeaders:
    """Verify that security headers are set by the rate-limit middleware.

    Note: The legacy SecurityHeadersMiddleware was removed (replaced by
    forge-shared SecurityMiddleware). These tests now cover the security
    headers set by SecureRateLimitMiddleware's enable_ddos_headers mode.
    """
