...
class User(SQLModel, table=True):
    ...
    # Team membership (populated when user joins an org via migration FK)
    # use_alter=True defers FK creation so circular org<->user FKs can be resolved
    team_id: UUID | None = Field(
        default=None,
        sa_column=Column(
            "team_id",
            sqlalchemy.dialects.postgresql.UUID(as_uuid=True),
            sqlalchemy.ForeignKey("organizations.id", use_alter=True, name="fk_users_team_id"),
            nullable=True,
            index=True,
        ),
    )
