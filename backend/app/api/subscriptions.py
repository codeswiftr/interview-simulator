...
Includes webhook event deduplication to prevent double-processing of
Stripe retries (implemented 2026-03).
"""
...
from forge_shared.analytics import track_stripe_conversion
...
from app.middleware.webhook_rate_limit import webhook_rate_limit
...
async def stripe_webhook(request: Request) -> dict[str, Any]:
    ...
    - invoice.payment_failed: Track failed payments for churn analysis

    Uses forge-shared billing webhook handling for signature verification
    and database-backed event deduplication to prevent double-processing.

    Args:
        request: FastAPI request with Stripe webhook payload

    Returns:
        Success response (200 OK for duplicates, to prevent Stripe retries)
    """
    ...
    async for db_session in get_session():
        try:
            ...
            if await is_event_processed(db_session, event_id):
            ...
            try:
                if event_type == "checkout.session.completed":
                    await _handle_checkout_completed(event_data, db_session, event_id)
                elif event_type == "customer.subscription.updated":
                    await _handle_subscription_updated(event_data, db_session, event_id)
                elif event_type == "customer.subscription.deleted":
                    await _handle_subscription_deleted(event_data, db_session, event_id)
                elif event_type == "invoice.paid":
                    await _handle_invoice_paid(event_data, db_session, event_id)
                elif event_type == "invoice.payment_failed":
                    await _handle_invoice_payment_failed(event_data, db_session, event_id)
                else:
                    logger.debug(f"Ignoring webhook event type: {event_type}")
                    # Mark ignored events as processed to prevent re-processing
                    from app.services.webhook_service import mark_event_processed
                ...
                logger.info(f"Processed Stripe webhook: {event_type} ({event_id})")
            except DuplicateEventError:
                # Race condition - another worker processed this event
                logger.info(f"Event processed by another worker: {event_id}")
                return {"status": "already_processed"}
...
async def _handle_checkout_completed(
    ...
    """Handle checkout.session.completed event with idempotency.

    Uses WebhookProcessor context manager to ensure event is only processed once.
    """
    from forge_shared.billing.models import SubscriptionStatus

    if not stripe_event_id:
        logger.warning("Missing stripe_event_id in checkout handler")
        return

    # Use WebhookProcessor for automatic deduplication
    async with WebhookProcessor(
        db=db_session,
        ...
        if not customer_id or not user_id:
            ...
            processor.set_result("skipped_missing_data")
            return
        ...
        try:
            ...
            processor.set_result("skipped_invalid_user_id")
            return
        user = result.first()
        if not user:
            logger.warning(f"User not found: {user_id}")
            processor.set_result("skipped_user_not_found")
            return
        ...
        if subscription_id:
            ...
            if subscription:
                ...
                # Track subscription created event via local Analytics service
                get_analytics().capture(
                    user_id=user_id,
                ...
                # Track Stripe conversion with UTM attribution via ConversionTracker
                try:
                    await track_stripe_conversion(
                        user_id=str(user.id),
                        stripe_event_id=stripe_event_id,
                        event_type="checkout_completed",
                        amount_usd=0.0,  # Amount not available in checkout.session.completed; invoice.paid carries the amount
                        plan_id=subscription_id,
                    )
                except Exception as e:
                    # Conversion tracking must not disrupt webhook processing
                    logger.warning(f"ConversionTracker failed for checkout_completed ({stripe_event_id}): {e}")

                # Mark as successfully processed
                processor.set_result(f"upgraded_to_{tier.value}", user.id)
...
async def _handle_subscription_updated(
    ...
    """Handle customer.subscription.updated event with idempotency.
    ...
    if not stripe_event_id:
        logger.warning("Missing stripe_event_id in subscription updated handler")
        return
    ...
    async with WebhookProcessor(
        ...
        if not customer_id:
            processor.set_result("skipped_missing_customer")
            return
        ...
        if subscription_id and client:
            ...
            if subscription:
                ...
                user.subscription_expires_at = subscription.current_period_end
                await db_session.flush()
                processor.set_result(f"updated_to_{tier.value}", user.id)
                return
        ...
        if status_raw:
            ...
            await db_session.flush()
            processor.set_result("updated_status_only", user.id)
...
async def _handle_subscription_deleted(
    ...
    """Handle customer.subscription.deleted event with idempotency."""
    if not stripe_event_id:
        logger.warning("Missing stripe_event_id in subscription deleted handler")
        return
    ...
    async with WebhookProcessor(
        ...
        # Capture previous tier before downgrade for analytics
        ...
        # Track subscription canceled event via local Analytics service
        get_analytics().capture(
            user_id=str(user.id),
        ...
        # Track Stripe conversion (cancellation) with UTM attribution via ConversionTracker
        try:
            await track_stripe_conversion(
                user_id=str(user.id),
                stripe_event_id=stripe_event_id,
            ...
            # Conversion tracking must not disrupt webhook processing
            logger.warning(f"ConversionTracker failed for subscription_canceled ({stripe_event_id}): {e}")

        processor.set_result(f"downgraded_from_{previous_tier}", user.id)
...
async def _handle_invoice_paid(
    ...
    """Handle invoice.paid event with idempotency.

    Fires when Stripe successfully charges the customer. Captures the actual
    payment amount for revenue attribution in PostHog via ConversionTracker.

    Args:
        event_data: Parsed invoice object from the webhook event.
        db_session: Active async database session.
        stripe_event_id: Stripe event ID for idempotent conversion tracking.
    """
    if not stripe_event_id:
        logger.warning("Missing stripe_event_id in invoice paid handler")
        return
    ...
    async with WebhookProcessor(
        ...
        # Support both forge-shared normalized format (customer_id) and raw Stripe
        # format (customer), following the same pattern as _handle_subscription_updated.
        customer_id = event_data.get("customer_id") or event_data.get("customer")
        ...
        if not customer_id:
            logger.warning("invoice.paid webhook missing customer identifier — skipping")
            processor.set_result("skipped_missing_customer")
            return
        ...
        amount_usd = amount_paid_cents / 100.0

        # Track via ConversionTracker for UTM/attribution data
        try:
            await track_stripe_conversion(
                user_id=str(user.id),
                stripe_event_id=stripe_event_id,
            ...
            # Conversion tracking must not disrupt webhook processing
            logger.warning(f"ConversionTracker failed for invoice.paid ({stripe_event_id}): {e}")

        # Track payment processed event via local Analytics service
        try:
            get_analytics().capture(
                user_id=str(user.id),
            ...
            logger.warning(f"Analytics capture failed for invoice.paid ({stripe_event_id}): {e}")

        processor.set_result(f"payment_recorded_${amount_usd:.2f}", user.id)
...
async def _handle_invoice_payment_failed(
    ...
    """Handle invoice.payment_failed event with idempotency.

    Fires when Stripe fails to charge the customer (declined card, expired card,
    insufficient funds). Tracks the failure in PostHog for churn analysis.

    Args:
        event_data: Parsed invoice object from the webhook event.
        db_session: Active async database session.
        stripe_event_id: Stripe event ID for idempotent event tracking.
    """
    if not stripe_event_id:
        logger.warning("Missing stripe_event_id in invoice payment_failed handler")
        return
    ...
    async with WebhookProcessor(
        ...
        customer_id = event_data.get("customer_id") or event_data.get("customer")
        amount_due_cents = event_data.get("amount_due", 0)
        subscription_id = event_data.get("subscription_id") or event_data.get("subscription")
        attempt_count = event_data.get("attempt_count", 1)

        if not customer_id:
            logger.warning("invoice.payment_failed webhook missing customer identifier — skipping")
            processor.set_result("skipped_missing_customer")
            return
        ...
        amount_usd = amount_due_cents / 100.0

        # Track payment failure in PostHog for churn/dunning analysis
        try:
            get_analytics().capture(
                user_id=str(user.id),
            ...
            logger.warning(
                f"Analytics capture failed for invoice.payment_failed ({stripe_event_id}): {e}"
            )

        logger.info(
            f"Payment failed for user {user.id}: ${amount_usd:.2f}, attempt {attempt_count}"
        )
        processor.set_result(f"payment_failed_recorded_${amount_usd:.2f}", user.id)
