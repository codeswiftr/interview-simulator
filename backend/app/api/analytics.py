...
def _require_admin_key(x_admin_key: str = Header(default="")) -> None:
    """Verify the X-Admin-Key header matches ADMIN_API_KEY env var."""
...
@router.get(
    "/admin/activation",
    summary="Activation metrics (admin only)",
    description=(
        "Returns weekly activated user counts. "
        "Requires X-Admin-Key header matching ADMIN_API_KEY env var. "
        "An activated user has completed at least one interview session (activated_at is set)."
    ),
)
async def get_activation_metrics(
    ...
    """Return count of users with activated_at set, split by 7-day and 30-day windows."""
    ...
    total_activated = (
        await db.exec(
            select(func.count(User.id)).where(User.activated_at.is_not(None))  # type: ignore[union-attr]
        )
    ).one()
    ...
    activated_7d = (
        await db.exec(
            select(func.count(User.id)).where(
                User.activated_at.is_not(None),  # type: ignore[union-attr]
                User.activated_at >= cutoff_7d,
            )
        )
    ).one()
