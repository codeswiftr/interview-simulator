...
export default function RecordingDeck({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Theme-aware visualizer colors
      const isDark = resolvedTheme === 'dark';
      const primaryColor = isDark ? '#38bdf8' : '#0ea5e9'; // sky-400 / sky-500
      const secondaryColor = isDark ? '#6366f1' : '#4f46e5'; // indigo-500 / indigo-600
  ...
  return (
    <div className="w-full bg-surface-primary dark:bg-surface-tertiary rounded-2xl shadow-xl overflow-hidden border border-border-light relative">
      {/* Visualizer Area */}
      <div className="relative h-48 bg-gradient-to-b from-slate-100 to-slate-200 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center overflow-hidden">
        {mediaStream ? (
          <canvas
            ref={canvasRef}
            width={600}
          ...
          <div className="text-slate-400 dark:text-white/20 flex flex-col items-center gap-2">
            <Mic size={48} />
        ...
        {isRecording && transcript && (
          <div className="absolute bottom-4 left-4 right-4 bg-white/80 dark:bg-black/60 backdrop-blur-sm p-3 rounded-lg border border-slate-200 dark:border-white/10">
            <p className="text-slate-800 dark:text-white/90 text-sm font-mono truncate animate-pulse">
              &gt; {transcript}
            </p>
          </div>
        )}
