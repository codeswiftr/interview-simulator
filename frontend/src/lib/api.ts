...
import type { TeamRead, TeamMemberRead, PublicInvitationRead, MemberUsageResponse } from '../types/team';
...
export interface VideoFeedbackRead {
  ...
  eye_contact_percentage: number; // 0.0 - 100.0
  video_url?: string;
}
...
export const videoAPI = {
  // Upload video file, returns VideoUploadResponse
  uploadVideo: async (responseId: string, videoBlob: Blob): Promise<VideoUploadResponse> => {
    const formData = new FormData();
    formData.append('file', videoBlob, 'video.webm');
    formData.append('response_id', responseId);

    const response = await api.post<VideoUploadResponse>('/upload/video', formData, {
      timeout: 120000, // 120 seconds for large video files
      headers: {
  ...
  // Get video feedback for a response. Returns null when feature flag is off (404).
  getVideoFeedback: async (responseId: string): Promise<VideoFeedbackRead | null> => {
    try {
      const response = await api.get<VideoFeedbackRead>(`/feedback/video/${responseId}`);
      return response.data;
    } catch (error) {
      // Feature flag VIDEO_FEATURES_ENABLED=false returns 404 — graceful degradation
  ...
  // Get per-response video metrics for an interview session.
  getVideoMetrics: (interviewId: string) =>
    api.get<Record<string, unknown>[]>(`/interviews/${interviewId}/video-metrics`),
};
...
export const teamAPI = {
  getMyTeam: (): Promise<TeamRead> =>
    api.get('/teams/me').then((r) => r.data),

  inviteMembers: (orgId: string, emails: string[]): Promise<{ sent: number }> =>
    api.post(`/teams/${orgId}/invitations`, { emails }).then((r) => r.data),

  removeMember: (orgId: string, userId: string): Promise<void> =>
    api.delete(`/teams/${orgId}/members/${userId}`).then((r) => r.data),

  revokeInvitation: (orgId: string, invitationId: string): Promise<void> =>
    api.delete(`/teams/${orgId}/invitations/${invitationId}`).then((r) => r.data),

  getInvitation: (token: string): Promise<PublicInvitationRead> =>
    api.get(`/teams/invitations/${token}`).then((r) => r.data),

  acceptInvitation: (token: string): Promise<TeamMemberRead> =>
    api.post(`/teams/invitations/${token}/accept`).then((r) => r.data),

  createTeam: (name: string, seatCount: number): Promise<TeamRead> =>
    api.post('/teams', { name, seat_count: seatCount }).then((r) => r.data),

  updateTeam: async (orgId: string, data: { name?: string; seat_count?: number }) => {
    const response = await api.patch(`/teams/${orgId}`, data);
    return response.data;
  },

  updateMemberRole: async (orgId: string, userId: string, role: 'admin' | 'member') => {
    const response = await api.patch(`/teams/${orgId}/members/${userId}/role`, { role });
    return response.data;
  },

  getTeamUsage: async (orgId: string) => {
    const response = await api.get(`/teams/${orgId}/usage`);
    return response.data as MemberUsageResponse[];
  },
};
