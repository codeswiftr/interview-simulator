...
import VideoFeedbackCard from '../components/interview/VideoFeedbackCard';
              ...
              <Card ref={heroRef} className="p-8 sm:p-12 text-center relative overflow-hidden">
                ...
                <div className="flex justify-center mb-8 relative z-10">
                  <div className="bg-[hsl(var(--muted)/0.3)] rounded-full p-6 border border-[hsl(var(--border))]">
                    <ScoreRing score={sessionFeedback.overall_score} size="large" responsive />
                  </div>
                </div>
                    ...
                    {responses.map((response, idx) => {
                      ...
                      return (
                        <div key={response.id} className="space-y-3">
                          ...
                          />
                          <VideoFeedbackCard responseId={response.id} />
                        </div>
                      );
                    })}
                  </div>
              ...
              <Card className="p-8 text-center">
                ...
                {user?.subscription_tier === 'free' && (
                  <div className="mt-8 pt-8 border-t border-border-light">
                    <div className="bg-gradient-to-r from-[#FF6B9D]/10 to-electric-blue/10 rounded-xl p-6 border border-[#FF6B9D]/20">
                      <div className="flex items-center justify-center gap-2 mb-3">
                        <Sparkles className="w-5 h-5 text-[#FF6B9D]" />
                        <span className="font-semibold text-text-primary">Unlock Unlimited Interviews</span>
                      </div>
                      <p className="text-sm text-text-secondary mb-4">
                        You&apos;ve used {user.interviews_this_month ?? 0} of 3 free interviews. Upgrade for unlimited practice + advanced feedback.
                      </p>
                      <button
                        onClick={() => navigate('/pricing')}
                        className="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg bg-[#FF6B9D] hover:bg-[#FF5588] text-white font-medium transition-colors shadow-md shadow-[#FF6B9D]/20"
                      >
