...
describe('HomePage', () => {
  ...
  describe('Hero Section Content', () => {
    it('displays the main headline with AI-powered gradient text', async () => {
      ...
      // Check for the headline text — "Interview Score" is in a span inside h1
      expect(screen.getByText(/Get Your First/i)).toBeInTheDocument();
      expect(screen.getByText(/Interview Score/i)).toBeInTheDocument();
    });
    ...
    it('displays the hero description text', async () => {
      await renderHomePage();
      expect(
        screen.getByText(/Pick a question, record your answer, and get instant feedback/i)
      ).toBeInTheDocument();
    });
  ...
  describe('CTA Buttons - Unauthenticated User', () => {
    it('displays "Start Practicing Free" button when not authenticated', async () => {
      ...
      // Button text changed to "Start Free — No Credit Card"
      const startButton = screen.getByRole('link', { name: /Start Free/i });
    ...
    it('hero CTA buttons have proper styling classes', async () => {
      ...
      // Primary CTA is "Start Free — No Credit Card"
      const startButton = screen.getByRole('link', { name: /Start Free/i });
    ...
    it('displays arrow icon in Start Practicing button', async () => {
      await renderHomePage(false);
      // Primary CTA link — find by role with partial text match
      const startButton = screen.getByRole('link', { name: /Start Free/i });
  ...
  describe('CTA Section', () => {
    it('displays CTA section heading', async () => {
      await renderHomePage();
      // CTA heading updated in component
      expect(screen.getByText('Know Your Score Before the Interview')).toBeInTheDocument();
    });

    it('displays CTA section description', async () => {
      await renderHomePage();
      // Description updated in component
      expect(
        screen.getByText(/Join 2,500\+ engineers who got interview-ready/i)
      ).toBeInTheDocument();
    });
  ...
  describe('Animation Classes', () => {
    ...
    it('has slide-up animations on hero content', async () => {
      ...
      // h1, p, button container, and free-trial p (4 elements) — plus any in FAQ/CTA
      expect(slideUpElements.length).toBeGreaterThanOrEqual(3);
    });
  ...
  describe('Accessibility', () => {
    ...
    it('links have accessible names', async () => {
      ...
      // Primary CTA text updated to "Start Free — No Credit Card"
