...
import { X, Zap, BarChart2, Target, Building2, MessageSquare, Terminal, Server, Layers, CheckCircle2, ChevronDown, Shuffle } from 'lucide-react';
...
export default function NewInterviewModal({ isOpen, onClose, onSubmit }: NewInterviewModalProps) {
  ...
  const handleRandomize = () => {
    ...
    setFormData((prev) => ({
      ...prev,
      interview_type: randomType.value as CreateInterviewFormData['interview_type'],
      difficulty: randomDifficulty.value as CreateInterviewFormData['difficulty'],
    }));
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      >
        ...
        {/* Footer */}
        <div className="p-6 border-t border-[hsl(var(--border))] flex justify-between items-center gap-3">
          <button
            type="button"
            onClick={handleRandomize}
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-text-secondary hover:text-text-primary hover:bg-[hsl(var(--muted))] transition-colors focus:outline-none focus:ring-2 focus:ring-electric-blue focus:ring-offset-1"
            disabled={isSubmitting}
            title="Pick a random interview type and difficulty"
          >
            <Shuffle size={16} />
            Randomize
          </button>
