...
interface InterviewContextValue {
  ...
  lastSubmittedResponseId: string | null;
  clearLastSubmittedResponseId: () => void;
...
export function InterviewProvider({ children }: InterviewProviderProps) {
  ...
  const [lastSubmittedResponseId, setLastSubmittedResponseId] = useState<string | null>(null);
  ...
  const handleSubmitAnswer = useCallback(async (blob: Blob, mimeType: string | null, duration: number) => {
    ...
    try {
      ...
      setLastSubmittedResponseId(responseData.id);
  ...
  const clearLastSubmittedResponseId = useCallback(() => {
    setLastSubmittedResponseId(null);
  }, []);
